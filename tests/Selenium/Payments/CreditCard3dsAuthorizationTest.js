...
describe('Credit Card 3-D Authorization Secure test', () => {
  before(async () => {
    driver = await getDriver('credit card 3ds authorization');
  });
  ...
  it('should check the credit card 3ds authorization payment process', async () => {
    await updateDatabaseTransactionType('reserve', 'payment/wirecardee_paymentgateway_creditcard/transaction_type');
    ...
    await clearDatabaseRows('authorization');
